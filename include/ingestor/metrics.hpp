...
class Metrics {
  public:
    ...
    void record_ingested(uint64_t count = 1) noexcept { m_ingested.fetch_add(count, std::memory_order_relaxed); }

    void record_dropped(uint64_t count = 1) noexcept { m_dropped.fetch_add(count, std::memory_order_relaxed); }

    void record_batch_sent(uint64_t count = 1) noexcept { m_batches_sent.fetch_add(count, std::memory_order_relaxed); }
    ...
    // Read counter values
    [[nodiscard]] uint64_t ingested() const noexcept { return m_ingested.load(std::memory_order_relaxed); }

    [[nodiscard]] uint64_t dropped() const noexcept { return m_dropped.load(std::memory_order_relaxed); }

    [[nodiscard]] uint64_t batches() const noexcept { return m_batches_sent.load(std::memory_order_relaxed); }
    ...
    [[nodiscard]] MetricsSnapshot snapshot() const noexcept {
        return MetricsSnapshot{.ingested = m_ingested.load(std::memory_order_relaxed),
    ...
    MetricsSnapshot exchange_and_reset() noexcept {
        return MetricsSnapshot{.ingested = m_ingested.exchange(0, std::memory_order_relaxed),
  ...
  private:
    ...
    alignas(kCacheLineSize) std::atomic<uint64_t> m_ingested{0};
    alignas(kCacheLineSize) std::atomic<uint64_t> m_dropped{0};
    alignas(kCacheLineSize) std::atomic<uint64_t> m_batches_sent{0};
